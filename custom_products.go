...
// GetAllProducts gets all product.product records that have been created after the time passed in.
...
func (c *Client) GetAllProducts(productsCreatedAfter time.Time) ([]byte, error) {
	criteria := NewCriteria()
	dtLayout := "2006-01-02 15:04:05"
	fmt.Printf("Looking for products created after %s", productsCreatedAfter.Format(dtLayout))
	criteria.Add("create_date", ">=", productsCreatedAfter.Format(dtLayout))
